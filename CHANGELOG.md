...
- `mostRecent` pagination now returns `offset`, `hasMore`, and `nextOffset` in the caller's newest-first coordinate space instead of the raw ascending API coordinates. Previously the first page (`offset=0`) reported `hasMore: false` (it maps to the API's _last_ page), so footer-driven paging stopped after one page, and feeding the footer's `nextOffset` back produced a two-page loop that kept serving the oldest items while everything newer stayed unreachable. Empty pages (`offset >= total`) no longer leak the internal count-probe's pagination fields. Affects all list tools sharing the pagination helper: `get_experiments`, `get_feature_flags`, `get_projects`, `get_data_sources`, `get_metrics`, and `get_sdk_connections`.
- The six paginated list tools above now use strict input schemas: unrecognized argument keys (e.g. `projectId` instead of `project`) produce a clear validation error naming the unknown keys, instead of being silently stripped — which made every pagination/filter field fall back to its default (`limit=100`, `offset=0`, `mostRecent=false`) and return the oldest 100 items as an apparent success.
- `get_experiments` and `get_feature_flags` list views now distinguish "no items exist" from "this page is empty" — an out-of-range offset returns the total and points back to `offset=0` instead of the misleading "No X found. Use create\_… to create one." (`get_data_sources` already did this.)
